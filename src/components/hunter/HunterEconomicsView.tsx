...
function CandidateEconomicsDetail({candidate,onChange}:{candidate:ProductCandidate;onChange:(c:ProductCandidate)=>void}){
  ...
  return <Card className="p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="font-display text-xl font-semibold">{candidate.name??"Producto sin nombre"}</h2><p className="text-xs text-brand-muted break-all">{candidate.sourceUrl}</p></div><PrimaryButton busy={busy} disabled={!score} onClick={()=>void act({op:"generate"})}>Generar landing</PrimaryButton></div>
    ...
    <div className="mt-5 flex flex-col md:flex-row gap-2"><input type="number" min="0.01" step="0.01" className="rounded-lg border border-brand-border bg-brand-surface px-3 py-2 text-sm" value={price} onChange={e=>setPrice(e.target.value)} placeholder="PVP obligatorio (€)"/><GhostButton disabled={busy||!(Number(price)>0)} onClick={()=>void act({op:"price",salePriceEur:Number(price)})}>Guardar PVP</GhostButton><select className="rounded-lg border border-brand-border bg-brand-surface px-3 py-2 text-sm" value={candidate.state} onChange={e=>void act({op:"state",state:e.target.value,note})}>{STATES.map(s=><option key={s}>{s}</option>)}</select><input className="min-w-0 flex-1 rounded-lg border border-brand-border bg-brand-surface px-3 py-2 text-sm" value={note} onChange={e=>setNote(e.target.value)} placeholder="Nota de decisión"/><GhostButton disabled={busy} onClick={()=>void act({op:"score"})}>Recalcular</GhostButton></div>
