...
export interface CodStoreRow { id: number; sweepId: number | null; pageId: string; pageName: string | null; country: string; priority: number | null; products: number; inDropea: number; strongWithoutSupplier: number; diversity: string | null; summary: string; audit: StoreAudit | null; requests: number; capturedAt: number }
...
export class CodHuntRepository {
  ...
  insertStore(a: StoreAudit, sweepId: number | null): number {
    const info = this.db.prepare("INSERT INTO hunter_cod_stores(sweep_id,page_id,page_name,country,priority,products,in_dropea,strong_without_supplier,diversity,summary,audit_json,requests,captured_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)")
      .run(sweepId, a.pageId, a.pageName, a.country, a.sweep?.priority ?? null, a.products.length, a.products.filter((p) => p.dropea.match).length, a.products.filter((p) => p.signal?.verdict === "senal_fuerte_sin_proveedor").length, a.account?.diversity.level ?? null, a.summary, JSON.stringify(a), a.requests, a.capturedAt);
    return Number(info.lastInsertRowid);
  }

  stores(opts: { limit?: number } = {}): CodStoreRow[] {
    const rows = this.db.prepare(`SELECT s.* FROM hunter_cod_stores s WHERE s.id = (SELECT x.id FROM hunter_cod_stores x WHERE x.page_id = s.page_id ORDER BY x.captured_at DESC, x.id DESC LIMIT 1) ORDER BY s.in_dropea DESC, s.strong_without_supplier DESC, s.priority DESC, s.captured_at DESC LIMIT ?`).all(opts.limit ?? 50) as Array<Record<string, unknown>>;
