...
import type { DiscoveryGroup, DiscoverySnapshot } from "./types";
  ...
  saveRun(input: {
    ...
    const tx = this.db.transaction(() => {
      ...
      for (const group of input.groups) {
        ...
        // Una sola fuente para la regla, y con su traza (docs/HUNTER-DISCOVERY-AUDITORIA.md).
        const trace = computeMomentum({
          activeAds: group.activeAds,
          previousActiveAds: previous?.active_ads ?? null,
          previousCapturedAt: previous?.captured_at ?? null,
          now: input.now,
        });
        const momentum = trace.status;
