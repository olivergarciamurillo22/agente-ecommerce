...
function range(sources: SearchEvidence[], consultedAt: number, expiresAt: number): PriceRange | null {
  ...
  return { min: round2(prices[0]), max: round2(prices.at(-1)!), probable: round2(prices.reduce((a, b) => a + b, 0) / prices.length), sources, consultedAt, expiresAt };
}
...
export async function estimatePredictiveCandidate(
  ...
  const wholesaleEstimate = { at100, at500, reason: at100 && at500 ? null : "sin fuentes mayoristas suficientes para ambos volumenes" };
  ...
  return { productQuery: query, competitorUrl, searchAvailable: true, searchMechanism: provider.mechanism, wholesale: wholesaleEstimate, retail: retailEstimate, viability, consultedAt: now, expiresAt };
}
