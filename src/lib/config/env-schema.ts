...
export const ENV_SCHEMA: EnvVarSpec[] = [
  ...
  // ── AUTO-DESPACHO TRAS COOLDOWN + IA DE INTENCIÓN — docs/AUTO-DESPACHO-COOLDOWN.md ──
  {
    ...
    secret: false,
    description: "1 = al confirmar se programa un cooldown (AUTO_DISPATCH_COOLDOWN_HOURS) y al vencer se despacha por el canal del producto (dispatch_channels: Beeping O Dropea, nunca ambos; sin canal = retenido) SOLO sin escaladas, cancelaciones ni ALERTA_DIRECCION abiertas; si no, queda 'DESPACHO RETENIDO' para revisión manual. 0 = comportamiento anterior (hook inmediato al confirmar).",
    requiredFor: [],
    mustEqual: { "local-safe": "0" },
    defaultValue: "0",
    validate: bool01,
  },
  { name: "AUTO_DISPATCH_COOLDOWN_HOURS", category: "AUTO_DISPATCH", secret: false, description: "Horas de espera tras la confirmación antes de despachar (1–168). Default 6 (Pedro, 07-09).", requiredFor: [], defaultValue: "6", validate: intPos },
  {
  ...
  // ── CAZADOR DE PRODUCTOS (Meta Ad Library, backend de Pedro) ──
  {
    ...
    category: "PRODUCT_HUNTER",
    secret: false,
    description: "off (default) | internal (backend dentro de este proceso: Ad Library, candidatos locales, catálogo de Dropea y cruces; sin URL) | api (backend externo con URL) | mock (SOLO desarrollo: en producción se rechaza). Sin definir, el módulo se muestra como no conectado.",
    requiredFor: [],
    defaultValue: "off",
    validate: enumOf("off", "internal", "api", "mock"),
  },
