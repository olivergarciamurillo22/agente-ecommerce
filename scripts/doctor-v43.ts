...
interface Result { name: string; status: Status; code: number; detail: string }
...
function run(block: Block): Result {
  ...
  return { name: block.name, status, code, detail: detail.slice(0, 140) };
}

console.log("\nCASAMABLE v4.3 · DOCTOR UNIFICADO\n");
const results = blocks.map(run);
console.log("BLOQUE".padEnd(34), "ESTADO", "DETALLE");
console.log("-".repeat(86));
for (const result of results) console.log(result.name.padEnd(34), result.status.padEnd(6), result.detail);
const failed = results.filter((result) => result.status === "FAIL");
const warned = results.filter((result) => result.status === "WARN");
console.log(`\nResultado: ${failed.length ? "FAIL" : warned.length ? "WARN" : "PASS"} · ${failed.length} fallo(s) · ${warned.length} aviso(s)\n`);
